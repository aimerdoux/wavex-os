 ...
 *    - "auth_failed" — login/org-access failure ("organization does not
 *                      have access … please login again"). This was the
 *                      DOMINANT failure on 2026-05-27 (174/192 = 91%),
 *                      model-agnostic (failed runs were already on opus),
 *                      so switching models does NOT fix it — re-auth does.
 *                      HALTs the spawn like "exhausted".
...
export type QuotaState = "ok" | "exhausted" | "auth_failed" | "transient" | "unknown";
...
export function classifyQuotaResponse(output: string): {
  ...
  // Auth/login failure — the dominant 2026-05-27 storm cause (174/192 = 91%):
  //   "subtype=success: Your organization does not have access to Claude.
  //    Please login again or contact your administrator"
  // claude exits success with this text, so it reads like a normal completion
  // unless inspected. Model-agnostic (failed runs were already on opus) — the
  // opus switch + quota gates do NOT cover it. Re-auth via `claude /login`.
  if (/does not have access/.test(text) || /please login again/.test(text)) {
    return { state: "auth_failed", resetHint: null };
  }
    ...
    child.on("close", () => {
      ...
      const detail =
        state === "exhausted"
          ...
          : state === "auth_failed"
            ? "Claude login invalid — organization does not have access; run `claude /login` to re-auth"
          : state === "transient"
