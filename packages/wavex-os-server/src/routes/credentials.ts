 ...
 *
 *  Endpoints:
 *    GET  /wavex-os/onboarding/credentials/:companyId
 *           Returns recommended connectors merged with vault state.
 *    POST /wavex-os/onboarding/credentials/paste
 *           { companyId, connectorId, key, plaintext } — vault-encrypts
 *    POST /wavex-os/onboarding/credentials/test
 *           { companyId, connectorId } — runs the connector probe
 *    POST /wavex-os/onboarding/credentials/skip
...
export function registerCredentialRoutes(app: FastifyInstance): void {
  ...
  app.post("/wavex-os/onboarding/credentials/paste", async (req, reply) => {
    if (!gateBoard(req, reply)) return;
  ...
  app.post("/wavex-os/onboarding/credentials/test", async (req, reply) => {
    if (!gateBoard(req, reply)) return;
  ...
  app.post("/wavex-os/onboarding/credentials/skip", async (req, reply) => {
    if (!gateBoard(req, reply)) return;
