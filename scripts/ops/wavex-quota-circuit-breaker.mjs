...
export function classifyQuotaResponse(output) {
  ...
  // Auth/login failure — dominant 2026-05-27 storm cause (91%), model-agnostic.
...
(async () => {
  ...
  if (state === "exhausted" || state === "auth_failed") {
    const reason = state === "auth_failed"
      ? "AUTH FAILED (org access / login)"
      : `quota EXHAUSTED (resets ${resetHint ?? "?"})`;
    if (tripped) {
      console.log(`[breaker] still ${state} (${MODEL}); fleet already paused since ${tripped.paused_at}.`);
      return;
    }
    console.log(`[breaker] ${MODEL} ${reason} — pausing fleet ${COMPANY}`);
    if (state === "auth_failed") console.log("[breaker] FIX: run `claude /login` / re-auth the keychain wrapper, then the breaker auto-resumes once a canary succeeds.");
    if (DRY_RUN) { console.log("[breaker] dry-run: would pause-fleet"); return; }
    const r = await fleetAction("pause").catch((e) => ({ error: String(e) }));
    writeMarker({ paused_at: now, model: MODEL, state, reset_hint: resetHint, result: r });
